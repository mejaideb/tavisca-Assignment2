...
namespace Tavisca.Bootcamp.LanguageBasics.Exercise1
{
    public static class Program
    {
        ...
        public static int[] FindMeal(int[] protein, int[] carbs, int[] fat, string[] dietPlans)
        {
            ...
            var res = new int[dietPlans.Length]; // will contain the indexes of menu to be selected respective to diet plan
            for (int i = 0; i < fat.Length; i++)
                cal[i] = protein[i] * 5 + carbs[i] * 5 + fat[i] * 9;
            for (int i = 0; i < dietPlans.Length; i++)
            {
                List<int> temp = new List<int>();
                foreach (char j in dietPlans[i]) // each element of dietPlan[i]
                {
                    if (temp.Count() == 0)
                    {
                        if (j == 'p')
